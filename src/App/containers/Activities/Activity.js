...
class Activity extends React.Component {
  ...
  componentDidMount() {
    ...
    api.get('/matter/viewforuser/' + _id).then((res) => {
      matters = res;
    ...
    if (timeError !== '') {
      ...
      if (this.state.editTime || this.state.EditExpense) {
        ...
        if (type === 'time') {
          ...
          data.userId = this.props.user._id;
          api
            .post('/activity/create', { ...data, userName: this.props.user.userName })
            .then((res) => {
              this.componentDidMount()
              this.setState({
                disableExpense: false,
  ...
  render() {
    ...
    const handleDelete = () => {
      ...
      if (selected && selected.length > 0) {
        ...
        notification.warning({ message: "Please select a activity" })
      }
    };
    ...
    const handleDublicate = () => {
      ...
      if (selected && selected.length > 0) {
        selected.map((record, index) => {
          if (record.type === 'time') {
            record.type = 'time';
            record.userId = this.props.user._id;
          ...
          //console.log(record)
          api
            .post('/activity/create', { ...record, userName: this.props.user.userName })
            .then((res) => {
              this.componentDidMount()
              this.setState({
                disableExpense: false,
          ...
          >
            ...
            >
              <div className="mb-2" style={{ "margin-top": "1rem" }}>
                ...
                <Button className="ml-2" onClick={() => handleDublicate()}>
                  Duplicate
                   </Button>
