...
class AddList extends React.Component {
  ...
  componentDidMount() {
    const { _id, isOwner, userName } = user.token.user
    api.get('/tasks/list/viewforuser/' + _id).then((res) => {
      let tableData = []
      console.log(res.data.data)
      res.data.data.map((value, index) => {
        if (isOwner || value.userName === userName) {
          const temp = {
            id: value._id,
            key: index,
            name: value.name,
  ...
  handleOk = () => {
    ...
    if (this.state.data.name == '') {
      ...
      if (this.state.editMode) {
        ...
        api.post('/tasks/list/create', payload).then((res) => {
          this.setState({
            data: {
              userId: user.token.user._id,
              name: '',
